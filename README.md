...
This gallery features **automated AI art generation** using Google's Gemini API:

- **Daily automated generation** of new artwork in various artistic styles
- **215 art styles** from classical to contemporary (Impressionism, Cubism, Art Deco, etc.)
- **Detailed art concepts** generated by AI and saved to a public gist
- **Images generated** from the art concept prompts using Gemini's Imagen API

### How Automated Generation Works

1. GitHub Actions runs daily at 6 AM UTC
2. Selects a random art style from 215 options
3. Generates a detailed 150-250 word art concept prompt using Gemini
4. Saves the art concept to a public gist for reference
5. Generates an image using the same prompt via Gemini Imagen API
6. Commits the new image and metadata to the repository
7. Triggers deployment to update the gallery

See [scripts/SETUP.md](scripts/SETUP.md) for API setup instructions.

## 🖼️ Manually Adding Images

You can also manually add images to the gallery:
